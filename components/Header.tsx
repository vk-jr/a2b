...
const NavLink: React.FC<{ link: NavLinkType; onClick?: () => void }> = ({ link, onClick }) => (
  <Link 
    to={link.path}
    onClick={onClick}
    className="text-slate-600 dark:text-medium hover:text-slate-900 dark:hover:text-light transition-colors duration-300 py-2"
  >
    {link.name}
  </Link>
);
...
const Header: React.FC = () => {
  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/80 dark:bg-secondary/80 backdrop-blur-lg' : 'bg-white dark:bg-primary'}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          <Link to="/" className="text-2xl font-display font-bold text-slate-900 dark:text-light">
          ...
          <div className="lg:hidden flex items-center space-x-4">
            <ThemeToggleButton />
            <button onClick={() => setIsOpen(!isOpen)} className="text-slate-800 dark:text-light focus:outline-none">

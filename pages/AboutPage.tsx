    ...
    <div>
      ...
      <Section>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
                <h2 className="text-3xl font-display font-bold text-slate-900 dark:text-light">Our Mission</h2>
                ...
                <h3 className="mt-8 text-2xl font-display font-bold text-slate-900 dark:text-light">Our Philosophy</h3>
      ...
      <Section className="bg-slate-50 dark:bg-secondary">
        <div className="text-center max-w-3xl mx-auto">
          <h2 className="text-3xl sm:text-4xl font-display font-bold text-slate-900 dark:text-light">Meet The Team</h2>
          <p className="mt-4 text-lg text-slate-600 dark:text-medium">Our success is driven by a dedicated team of AI engineers, data scientists, and business strategists passionate about solving complex problems.</p>
        </div>
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {TEAM_MEMBERS.map((member) => (
                <div key={member.name} className="text-center bg-white dark:bg-primary p-6 rounded-lg shadow-md">
                    ...
                    <p className="mt-2 text-sm text-slate-600 dark:text-medium">{member.bio}</p>
      ...
      <Section>
         <div className="text-center max-w-3xl mx-auto">
            <h2 className="text-3xl sm:text-4xl font-display font-bold text-slate-900 dark:text-light">How We Work</h2>
            <p className="mt-4 text-lg text-slate-600 dark:text-medium">Our proven 5-step process ensures we deliver high-impact AI solutions that are perfectly aligned with your business objectives.</p>
        </div>
        <div className="mt-16 max-w-4xl mx-auto">
            {PROCESS_STEPS.map((step) => (
                <div key={step.step} className="flex flex-col sm:flex-row items-start my-8">
                    ...
                    <div>
                        <h3 className="text-xl font-bold font-display text-slate-900 dark:text-light">{step.title}</h3>
                        <p className="mt-1 text-slate-600 dark:text-medium">{step.description}</p>
                    </div>
                </div>
            ))}
        </div>
